...
__global__ void matrixMultiplyShared(float *A, float *B, float *C, int numARows,
  ...
  for(int t = 0; t < Grid_Size(numAColumns); t++)
  {
	if(row < numARows && TILE_WIDTH * t + tx < numAColumns)
		ds_A[ty][tx] = A[row * numAColumns + TILE_WIDTH * t + tx];
	else
		ds_A[ty][tx] = 0.0;
	if(TILE_WIDTH * t + ty < numBRows && col < numBColumns)
		ds_B[ty][tx] = B[(TILE_WIDTH * t + ty) * numBColumns + col];
	else
		ds_B[ty][tx] = 0.0;		
	__syncthreads();
...
int main(int argc, char **argv) {
  ...
  dim3 DimGrid(Grid_Size(numCColumns), Grid_Size(numCRows), 1);
